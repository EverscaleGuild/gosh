...
// Gosh contract
async function getRepoAddress(repoName, pubkey) {
    const result = await runLocal(Gosh, 'getAddrRepository', { pubkey, name: repoName })
    return result.decoded.output.value0
}

async function getRepo(repoName = CURRENT_REPO, pubkey = userWallet().keys.public) {
    const repo = {
        ...Repository,
        name: repoName,
        address: await getRepoAddress(repoName, `0x${pubkey}`)
    }
