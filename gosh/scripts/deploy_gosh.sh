...
$TONOS_CLI -u $NETWORK call $GOSH_ADDR setRepository "{\"code\":\"$REPO_CODE\"}" --abi $fn_abi --sign $fn_keys > /dev/null || exit 1

echo "load \`commit\`-contract"
COMMIT_CODE=$($TVM_LINKER decode --tvc ../commit.tvc | sed -n '/code:/ s/ code: // p')
$TONOS_CLI -u $NETWORK call $GOSH_ADDR setCommit "{\"code\":\"$COMMIT_CODE\"}" --abi $fn_abi --sign $fn_keys > /dev/null || exit 1

echo "load \`blob\`-contract"
BLOB_CODE=$($TVM_LINKER decode --tvc ../blob.tvc | sed -n '/code:/ s/ code: // p')
$TONOS_CLI -u $NETWORK call $GOSH_ADDR setBlob "{\"code\":\"$BLOB_CODE\"}" --abi $fn_abi --sign $fn_keys > /dev/null || exit 1

echo "load \`wallet\`-contract"
WALLET_CODE=$($TVM_LINKER decode --tvc ../goshwallet.tvc | sed -n '/code:/ s/ code: // p')
$TONOS_CLI -u $NETWORK call $GOSH_ADDR setWallet "{\"code\":\"$WALLET_CODE\"}" --abi $fn_abi --sign $fn_keys > /dev/null || exit 1

echo "load \`dao\`-contract"
DAO_CODE=$($TVM_LINKER decode --tvc ../goshdao.tvc | sed -n '/code:/ s/ code: // p')
$TONOS_CLI -u $NETWORK call $GOSH_ADDR setDao "{\"code\":\"$DAO_CODE\"}" --abi $fn_abi --sign $fn_keys > /dev/null || exit 1

echo "load \`tag\`-contract"
TAG_CODE=$($TVM_LINKER decode --tvc ../tag.tvc | sed -n '/code:/ s/ code: // p')
$TONOS_CLI -u $NETWORK call $GOSH_ADDR setTag "{\"code\":\"$TAG_CODE\"}" --abi $fn_abi --sign $fn_keys > /dev/null || exit 1
...
$TONOS_CLI -u $NETWORK call $GOSH_ADDR setTokenWallet "{\"code\":\"$TokenWallet_CODE\"}" --abi $fn_abi --sign $fn_keys > /dev/null || exit 1

echo "load \`SMVTokenLocker\`-contract"
SMVTokenLocker_CODE=$($TVM_LINKER decode --tvc ../../smv/SMVTokenLocker.tvc | sed -n '/code:/ s/ code: // p')
$TONOS_CLI -u $NETWORK call $GOSH_ADDR setTokenLocker "{\"code\":\"$SMVTokenLocker_CODE\"}" --abi $fn_abi --sign $fn_keys > /dev/null || exit 1

echo "load \`LockerPlatform\`-contract"
LockerPlatform_CODE=$($TVM_LINKER decode --tvc ../../smv/LockerPlatform.tvc | sed -n '/code:/ s/ code: // p')
$TONOS_CLI -u $NETWORK call $GOSH_ADDR setSMVPlatform "{\"code\":\"$LockerPlatform_CODE\"}" --abi $fn_abi --sign $fn_keys > /dev/null || exit 1

echo "load \`SMVClient\`-contract"
SMVClient_CODE=$($TVM_LINKER decode --tvc ../../smv/SMVClient.tvc | sed -n '/code:/ s/ code: // p')
$TONOS_CLI -u $NETWORK call $GOSH_ADDR setSMVClient "{\"code\":\"$SMVClient_CODE\"}" --abi $fn_abi --sign $fn_keys > /dev/null || exit 1

echo "load \`SMVProposal\`-contract"
SMVProposal_CODE=$($TVM_LINKER decode --tvc ../../smv/SMVProposal.tvc | sed -n '/code:/ s/ code: // p')
$TONOS_CLI -u $NETWORK call $GOSH_ADDR setSMVProposal "{\"code\":\"$SMVProposal_CODE\"}" --abi $fn_abi --sign $fn_keys > /dev/null || exit 1
